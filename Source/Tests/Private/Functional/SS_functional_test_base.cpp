...
ASS_functional_test_base::ASS_functional_test_base()
{
...
auto ASS_functional_test_base::BeginPlay() -> void
{
    Super::BeginPlay();

    // Initialize necessary game references.
    player_character_ref_ = Cast<ASS_Player_character>(UGameplayStatics::GetPlayerPawn(GetWorld(), 0));
    game_mode_ref_        = Cast<ASS_Game_mode_kill_em_all>(UGameplayStatics::GetGameMode(GetWorld()));
    controller_ref_       = Cast<ASS_Player_controller>(UGameplayStatics::GetPlayerController(GetWorld(), 0));

    input_subsystem_ref_  = ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(controller_ref_->GetLocalPlayer());
    
}

auto ASS_functional_test_base::Tick(float delta_time) -> void
{
    Super::Tick(delta_time);
}

auto ASS_functional_test_base::RunTest(const TArray<FString>& params) -> bool
{
...
auto ASS_functional_test_base::setup() -> void
{
    // No Tank ref test needs to abort.
    if (!player_character_ref_)
    {
        UE_LOG(LogTemp, Error, TEXT("FUNCTIONAL TEST ERROR: NO CHARACTER REF!"));
        abort_test();
        return;
    }
...
auto ASS_functional_test_base::progress_test_step() -> void
{
...
auto ASS_functional_test_base::wait() -> void
{
...
auto ASS_functional_test_base::do_player_ia(UInputAction*                       input_action, 
...
auto ASS_functional_test_base::set_mouse_pos(float x, float y) -> void
{
    if (controller_ref_)
    {
...
auto ASS_functional_test_base::abort_test() -> void
{
...
auto ASS_functional_test_base::finish_test() -> void
{
...
auto ASS_functional_test_base::FinishTest(EFunctionalTestResult test_result, const FString& message) -> void
{
